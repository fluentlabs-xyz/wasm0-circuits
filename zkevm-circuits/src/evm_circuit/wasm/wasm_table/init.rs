...
#[derive(Clone, Debug)]
pub(crate) struct WasmTableInitGadget<F> {
...
impl<F: Field> ExecutionGadget<F> for WasmTableInitGadget<F> {
...
/*
        cb.condition(is_copy_op.expr(), |cb| {
            ...
            cb.stack_pop(dst_idx.expr());
            cb.stack_pop(range.expr());
            cb.table_init(table_index.expr(), table_index_rhs.expr(), src_idx.expr(), dst_idx.expr(), range_idx.expr());
        });
*/
