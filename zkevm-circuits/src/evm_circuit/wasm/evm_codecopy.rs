...
impl<F: Field> ExecutionGadget<F> for EvmCodeCopyGadget<F> {
    ...
    fn configure(cb: &mut EVMConstraintBuilder<F>) -> Self {
        ...
        let dst_memory_addr = MemoryAddress64Gadget::construct(cb, dst_memory_offset, size);
...
mod tests {
    ...
    fn codecopy_gadget_simple() {
        test_ok(0x00u32, 0x00u32, 0x20u32, false);
        test_ok(0x30u32, 0x20u32, 0x30u32, false);
        test_ok(0x20u32, 0x10u32, 0x42u32, false);
    }

    #[test]
    fn codecopy_gadget_large() {
        test_ok(0x102u32, 0x103u32, 0x101u32, true);
    }

    #[test]
    fn codecopy_gadget_code_offset_overflow() {
        test_ok(u32::MAX, 0x103u32, 0x101, true);
    }
