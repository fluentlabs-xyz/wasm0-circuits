...
/// TODO mem_segment_type(1) -> mem_segment_size_opcode(1) -> mem_segment_size+ -> ??? (usually value: 0xB) -> mem_segment_len+ -> mem_segment_bytes+
#[derive(Clone)]
pub struct WasmDataSectionBodyItemDescriptor {
...
#[derive(Clone)]
pub struct WasmDataSectionBodyDescriptor {
    pub items: Vec<WasmDataSectionBodyItemDescriptor>,
}

pub fn generate_wasm_data_section_item_bytecode(descriptor: &WasmDataSectionBodyItemDescriptor) -> Vec<u8> {
...
// items_count+ -> item+ (mem_segment_type(1) -> mem_segment_size_opcode(1) -> mem_segment_size+ -> ??? (usually value: 0xB) -> mem_segment_len+ -> mem_segment_bytes+)
...
pub fn generate_wasm_data_section_body_bytecode(descriptor: &WasmDataSectionBodyDescriptor) -> Vec<u8> {
    let items_count = descriptor.items.len();
    let mut bytecode: Vec<u8> = vec![];
    bytecode.extend(leb128_convert(false, items_count as i128));
    for item in &descriptor.items {
        bytecode.extend(generate_wasm_data_section_item_bytecode(item));
    }
...
mod test_helpers {
    ...
    #[test]
    pub fn generate_wasm_data_section_body_bytecode_test() {
        // expected
