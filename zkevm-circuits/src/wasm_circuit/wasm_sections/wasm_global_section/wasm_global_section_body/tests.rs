...
impl<'a, F: Field> Circuit<F> for TestCircuit<'a, F> {
    ...
    fn configure(
        ...
        let test_circuit_config = TestCircuitConfig {
            wasm_global_section_body_chip: Rc::new(wasm_global_section_body_chip),
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                config.wasm_global_section_body_chip.config.leb128_chip.assign_init(&mut region, self.bytecode.len() - 1);
                config.wasm_global_section_body_chip.assign_init(&mut region, self.bytecode.len() - 1);
                config.wasm_global_section_body_chip.assign_auto(
                    &mut region,
                    &wasm_bytecode,
                    self.offset_start,
                ).unwrap();
...
mod wasm_global_section_body_tests {
    ...
    use crate::wasm_circuit::wasm_sections::wasm_global_section::wasm_global_section_body::tests::TestCircuit;
