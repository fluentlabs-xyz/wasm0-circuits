...
impl<'a, F: Field> Circuit<F> for TestCircuit<'a, F> {
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                config.wasm_memory_section_body_chip.config.leb128_chip.assign_init(&mut region, self.bytecode.len() - 1);
                config.wasm_memory_section_body_chip.assign_init(&mut region, self.bytecode.len() - 1);
