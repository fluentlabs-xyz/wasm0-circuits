...
impl<F: Field> LEB128Chip<F> {
    ...
    pub fn configure(cs: &mut ConstraintSystem<F>, bytes: &Column<Advice>) -> LEB128Config<F> {
        ...
        cs.create_gate("LEB128 gate", |vc| {
            ...
            let is_consider_byte_expr = select::expr(
                ...
                vc.query_fixed(is_byte_has_cb, Rotation::prev()) + is_byte_has_cb_expr.clone()
                    - vc.query_fixed(is_byte_has_cb, Rotation::prev())
            ...
            cb.condition(is_first_byte_expr.clone(), |cb| {
                cb.require_zero(
            ...
            cb.condition(
                and::expr([
                    not::expr(is_first_byte_expr.clone()),
                    or::expr([is_byte_has_cb_expr.clone(), is_last_byte_expr.clone()]),
                ]),
                |cb| {
                    let leb_byte_mul_prev_expr =
                        vc.query_advice(byte_mul.clone(), Rotation::prev());
                    cb.require_equal(
                        "leb_byte_mul growth",
            ...
            cb.condition(not::expr(is_first_byte_expr.clone()), |cb| {
            ...
            cb.condition(is_last_byte_expr.clone(), |cb| {
                cb.require_zero("byte_has_cb is 0 on last_byte", is_byte_has_cb_expr.clone());
            });
            ...
            cb.condition(
                ...
                |cb| {
                    let mut sn_recovered_manual_expr = (byte_val_expr.clone()
                        - 0b10000000.expr() * is_byte_has_cb_expr.clone())
                        * leb_byte_mul_expr.clone();
                    ...
                    sn_recovered_manual_expr =
                        sn_recovered_manual_expr + sn_recovered_prev_expr.clone();
                    cb.require_equal(
            ...
            cb.condition(not::expr(is_first_byte_expr.clone()), |cb| {
                ...
                cb.require_zero(
                    "is_first_byte => prev.sn=next.sn",
                    sn_expr.clone() - sn_prev_expr.clone(),
                );
            });
            cb.condition(is_last_byte_expr.clone(), |cb| {
                cb.require_equal(
                    "is_last_byte => sn=sn_recovered",
                    sn_expr.clone(),
                    sn_recovered_expr.clone(),
                );
            });
            cb.condition(not::expr(is_consider_byte_expr.clone()), |cb| {
                cb.require_zero(
    ...
    pub fn assign(
        ...
        let leb_byte_mul = if p.is_byte_has_cb() || p.is_last_byte() {
            pow(0b10000000, p.byte_rel_offset)
        } else {
            0
        };
        region
            .assign_advice(
                || {
                    format!(
                        "assign 'leb_byte_mul' to {} at {}",
                        leb_byte_mul, assign_offset
                    )
                },
        ...
        region
            .assign_advice(
                || {
                    format!(
                        "assign 'sn' is_signed '{}' to {} at {}",
                        p.is_signed, p.sn, assign_offset
                    )
                },
                self.config.sn,
                assign_offset,
        ...
        region
            .assign_advice(
                || {
                    format!(
                        "assign 'sn_recovered' is_signed '{}' to {} at {}",
                        p.is_signed, p.sn_recovered_at_pos, assign_offset
                    )
                },
