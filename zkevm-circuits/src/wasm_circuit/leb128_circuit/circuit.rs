...
/// LEB128Config
#[derive(Debug, Clone)]
pub struct LEB128Config<F, const BIT_DEPTH: usize, const IS_SIGNED: bool> {
...
impl<F: Field, const BIT_DEPTH: usize, const IS_SIGNED: bool> LEB128Config<F, BIT_DEPTH, IS_SIGNED>
{}

///
#[derive(Debug, Clone)]
pub struct LEB128Chip<F, const BIT_DEPTH: usize, const IS_SIGNED: bool> {
    ///
    pub config: LEB128Config<F, BIT_DEPTH, IS_SIGNED>,
    _marker: PhantomData<F>,
}

impl<F: Field, const BIT_DEPTH: usize, const IS_SIGNED: bool> LEB128Chip<F, BIT_DEPTH, IS_SIGNED>
{
    /// max leb bytes needed to represent number with selected BIT_DEPTH
    pub const LEB_BYTES_N: usize = (BIT_DEPTH + 6) / 7;

    ///
    pub fn construct(config: LEB128Config<F, BIT_DEPTH, IS_SIGNED>) -> Self {
        let instance = Self {
            config,
            _marker: PhantomData,
        };
    ...
    ) -> LEB128Config<F, BIT_DEPTH, IS_SIGNED> {
        Self::validate_static_state();
