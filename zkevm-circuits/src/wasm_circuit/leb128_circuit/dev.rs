...
use crate::wasm_circuit::leb128_circuit::helpers::leb_compute_sn_recovered_at_position;
...
impl<'a, F: Field, const IS_SIGNED: bool> Circuit<F> for TestCircuit<'a, F, IS_SIGNED> {
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            || "leb128 region",
            |mut region| {
                leb128_chip.assign_init(&mut region, self.leb_bytes.len() - 1);
                ...
                for (leb_byte_offset, &leb_byte) in self.leb_bytes.iter().enumerate() {
                    ...
                    sn_recovered_at_pos = leb_compute_sn_recovered_at_position(
                        sn_recovered_at_pos,
                        self.is_signed,
                        leb_byte_offset,
                        leb_byte as u64,
                    ...
                    leb128_chip.assign(
                        ...
                        is_first_leb_byte,
                        is_last_leb_byte,
                        leb_byte_offset < (self.leb_bytes_last_byte_index as usize),
                        self.is_signed,
                        self.sn,
                        sn_recovered_at_pos,
                    );
...
mod leb128_circuit_tests {
    ...
    pub fn test_leb_broken_continuation_bit_signed() {
        const IS_SIGNED: bool = true;
        leb_broken_continuation_bit::<1, { IS_SIGNED }>();
        // leb_broken_continu<ation_bit::<2, { IS_SIGNED }>();
        // leb_broken_continuation_bit::<3, { IS_SIGNED }>();
        // leb_broken_continuation_bit::<4, { IS_SIGNED }>();
        // leb_broken_continuation_bit::<5, { IS_SIGNED }>();
        // leb_broken_continuation_bit::<6, { IS_SIGNED }>();
