...
use crate::wasm_circuit::specs::{host_function::HostPlugin, itable::OpcodeClass};
use std::{
    marker::PhantomData,
    ops::{Add, Mul},
};
...
#[derive(Clone)]
pub struct InputTableConfig<F: Field> {
    enable: Column<Fixed>,
    index: Column<Fixed>,
...
impl<F: Field> InputTableConfig<F> {
...
impl<F: Field> ForeignTableConfig<F> for InputTableConfig<F> {
...
pub struct InputTableChip<F: Field> {
    config: InputTableConfig<F>,
}

impl<F: Field> InputTableChip<F> {
