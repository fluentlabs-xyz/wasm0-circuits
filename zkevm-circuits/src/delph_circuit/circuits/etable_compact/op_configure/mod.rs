use crate::delph_circuit::circuits::{config::POW_TABLE_LIMIT, rtable::offset_len_bits_encode};
...
impl UnlimitedCell {
    pub fn assign<F: Field>(&self, ctx: &mut Context<'_, F>, value: F) -> Result<(), Error> {
    ...
    pub fn expr<F: Field>(&self, meta: &mut VirtualCells<'_, F>) -> Expression<F> {
...
impl PowTableLookupCell {
    pub fn assign<F: Field>(&self, ctx: &mut Context<'_, F>, power: u64) -> Result<(), Error> {
...
impl BitCell {
    pub fn assign<F: Field>(&self, ctx: &mut Context<'_, F>, value: bool) -> Result<(), Error> {
        ctx.region.assign_advice(
            || "bit cell",
            self.col,
            (ctx.offset as i32 + self.rot) as usize,
...
impl U4BopCell {
    ...
    pub fn eq_constraint<F: Field>(&self, meta: &mut VirtualCells<'_, F>) -> Expression<F> {
...
impl U64Cell {
    ...
    pub fn u4_expr<F: Field>(&self, meta: &mut VirtualCells<'_, F>, i: i32) -> Expression<F> {
...
impl U64OnU8Cell {
    ...
    pub fn u8_expr<F: Field>(&self, meta: &mut VirtualCells<'_, F>, i: i32) -> Expression<F> {
...
pub trait EventTableOpcodeConfigBuilder<F: Field> {
    fn configure(
...
pub trait EventTableForeignOpcodeConfigBuilder<F: Field> {
    fn configure(
        common: &mut EventTableCellAllocator<F>,
        constraint_builder: &mut ConstraintBuilder<F>,
    ) -> Box<dyn EventTableOpcodeConfig<F>>;
...
pub trait EventTableOpcodeConfig<F: Field> {
