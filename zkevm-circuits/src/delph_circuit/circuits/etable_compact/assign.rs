...
use eth_types::Field;
use crate::delph_circuit::circuits::config::max_etable_rows;

use super::*;

impl<F: Field> EventTableCommonConfig<F> {
    pub(super) fn assign(
        &self,
        ctx: &mut Context<'_, F>,
        ...
        for i in 0..max_etable_rows() as usize {
            ctx.region
                .assign_fixed(|| "etable common sel", self.sel, i, || Value::known(F::one()))?;
        ...
        macro_rules! assign_advice {
            ($c:expr, $o:expr, $k:expr, $v:expr) => {
                ctx.region
                    .assign_advice(|| $k, $c, ctx.offset + $o as usize, || Value::known(F::from($v)))?
            };
        }
