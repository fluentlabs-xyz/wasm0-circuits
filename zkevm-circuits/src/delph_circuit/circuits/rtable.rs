...
use crate::delph_circuit::traits::circuits::bit_range_table::BitRangeTable;
...
use crate::delph_circuit::specs::itable::BitOp;
...
#[derive(Clone)]
pub struct RangeTableConfig<F: Field> {
    // [0 .. 65536)
    u16_col: TableColumn,
    // [0 .. 256)
    u8_col: TableColumn,
...
impl<F: Field> RangeTableConfig<F> {
...
pub struct RangeTableChip<F: Field> {
...
pub fn offset_len_bits_encode_expr<F: Field>(
    offset: Expression<F>,
    len: Expression<F>,
    bits: Expression<F>,
) -> Expression<F> {
                    ...
                    for l in 0..1 << 4 {
                        for r in 0..1 << 4 {
                            ...
                            table.assign_cell(
                                ...
                                offset as usize,
                                || {
                                    Value::known(F::from((l * 256 + r * 16 + res) as u64)
                                        * bn_to_field::<F>(
            ...
            |mut table| {
                ...
                for i in 0..POW_TABLE_LIMIT {
                    table.assign_cell(
                        ...
                        offset as usize,
                        || {
                            Value::known(bn_to_field::<F>(&(BigUint::from(1u64) << (i + 16)))
                                + F::from(i as u64))
                        },
                    )?;
                    offset += 1;
...
impl<F: Field> BitRangeTable<F> for RangeTableConfig<F> {
