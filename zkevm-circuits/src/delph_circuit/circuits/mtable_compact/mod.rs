...
use crate::delph_circuit::circuits::{mtable_compact::configure::STEP_SIZE, IMTABLE_COLUMNS};
use halo2_proofs::{
    circuit::Cell,
...
use eth_types::Field;
use crate::delph_circuit::specs::mtable::{AccessType, InitType, LocationType, MTable, MemoryTableEntry, VarType};
...
#[derive(Clone)]
pub struct MemoryTableConfig<F: Field> {
...
impl<F: Field> MemoryTableConfig<F> {
    pub fn configure(
...
pub struct MemoryTableChip<F: Field> {
    config: MemoryTableConfig<F>,
}

impl<F: Field> MemoryTableChip<F> {
    ...
    pub fn assign(
        ...
        for i in 0..mtable_rows() {
            ctx.region
                .assign_fixed(|| "mtable sel", self.config.sel, i, || Value::known(F::one()))?;
