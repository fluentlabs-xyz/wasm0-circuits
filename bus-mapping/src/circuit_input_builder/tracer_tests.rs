...
use eth_types::{address, bytecode, evm_types::{stack::Stack, Gas, OpcodeId}, geth_types::GethData, word, Bytecode, Hash, ToAddress, ToWord, Word, StackWord, ToStackWord};
...
impl CircuitInputBuilderTx {
    fn new(geth_data: &GethData, geth_step: &GethExecStep) -> Self {
        let block = crate::mocks::BlockData::new_from_geth_data(geth_data.clone());
...
fn tracer_err_address_collision() {
    ...
    builder.state_ref().call_ctx_mut().unwrap().memory = memory[0].clone();
...
fn check_err_invalid_code(step: &GethExecStep, next_step: Option<&GethExecStep>) -> bool {
    ...
    step.op == OpcodeId::RETURN
        ...
        && step.memory[0].0.get(offset.low_u64() as usize) == Some(&0xef)
}
...
fn tracer_err_invalid_code() {
    ...
    builder.tx_ctx.call_is_success.push(false);
    builder.state_ref().push_call(mock_internal_create());
    builder.state_ref().call_ctx_mut().unwrap().memory = step.memory[0].clone();
    assert_eq!(
        builder.state_ref().get_step_err(step, next_step).unwrap(),
